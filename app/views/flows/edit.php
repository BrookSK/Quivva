...
?>
<h3>Editar Fluxo</h3>
<form method="post" action="/flows/update/<?= (int)$flow['id'] ?>" onsubmit="return saveDefinition();">
  <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($csrf) ?>">
  <div class="mb-3">
    <label class="form-label">Nome do fluxo</label>
    <input class="form-control" type="text" name="name" value="<?= htmlspecialchars($flow['name']) ?>" required>
  </div>
  <div class="row g-3">
    <div class="col-md-4">
    ...
    <div class="col-md-8">
      <div class="card h-100">
        ...
        <div id="flow-canvas" class="card-body min-vh-25" style="min-height:300px; background:#fff;">
          <?php foreach ($blocks as $b): if (($b['type'] ?? '') === 'text'): ?>
            <div class="border rounded p-2 mb-2" data-type="text">
              ...
              <div class="mt-2">
                <textarea class="form-control" placeholder="Mensagem de texto, ex: Olá {{nome}}"><?= htmlspecialchars($b['text'] ?? '') ?></textarea>
              </div>
            </div>
          <?php elseif (($b['type'] ?? '') === 'question'): ?>
            <div class="border rounded p-2 mb-2" data-type="question">
              <div class="d-flex justify-content-between align-items-center">
                <strong>Pergunta</strong>
                <button type="button" class="btn btn-sm btn-link text-danger" onclick="this.closest('div.border').remove()">remover</button>
              </div>
              <div class="row g-2 mt-2">
                <div class="col-md-6"><input class="form-control" placeholder="Nome da variável, ex: produto" value="<?= htmlspecialchars($b['variable'] ?? '') ?>"></div>
                <div class="col-md-6"><input class="form-control" placeholder="Pergunta, ex: Qual produto?" value="<?= htmlspecialchars($b['text'] ?? '') ?>"></div>
              </div>
            </div>
